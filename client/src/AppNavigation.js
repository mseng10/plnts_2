...
function AppNavigation({ window }) {
    ...
    // Destructure fetchMeta to allow for periodic updates.
    const { meta, fetchMeta } = useMeta();

    // Set up an interval to refetch meta data every minute for live badge updates.
    useEffect(() => {
        if (fetchMeta) {
