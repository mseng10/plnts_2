...
def manage_plant_alerts() -> None:
    ...
    try:
        ...
        plants_with_alerts: Dict[str, Alert] = {str(alert.model_id): alert for alert in existing_alerts}
        ...
        for plant in plants:
            ...
            care_plan: CarePlan = care_plans.get(str(plant.care_plan_id)) if plant.care_plan_id else None
            ...
            try:
                ...
                if not _has_alert_type(plants_with_alerts, plant_id_str, AlertTypes.WATER):
                    if care_plan.watering and plant.watered_on:
                        ...
                        if water_due_date < now:
                            new_alert = Alert(model_id=plant.id, alert_type=AlertTypes.WATER)
                            Table.ALERT.create(new_alert)
                            alerts_created_count += 1
                            logger.info(f"Created watering alert for plant: ID: {plant.id}")
                
                # Check for fertilize alert
                if not _has_alert_type(plants_with_alerts, plant_id_str, AlertTypes.FERTILIZE):
                    if care_plan.fertilizing and plant.fertilized_on:
                        ...
                        if fertilize_due_date < now:
                            new_alert = Alert(model_id=plant.id, alert_type=AlertTypes.FERTILIZE)
                            Table.ALERT.create(new_alert)
                            alerts_created_count += 1
                            logger.info(f"Created fertilize alert for plant: ID: {plant.id}")
                
                # Check for repot alert
                if not _has_alert_type(plants_with_alerts, plant_id_str, AlertTypes.REPOT):
                    if care_plan.potting and plant.potted_on:
                        ...
                        if potting_due_date < now:
                            new_alert = Alert(model_id=plant.id, alert_type=AlertTypes.REPOT)
                            Table.ALERT.create(new_alert)
                            alerts_created_count += 1
                            logger.info(f"Created repot alert for plant: ID: {plant.id}")
                
                # Check for cleanse alert
                if not _has_alert_type(plants_with_alerts, plant_id_str, AlertTypes.CLEANSE):
                    if care_plan.cleaning and plant.cleansed_on:
                        ...
                        if cleaning_due_date < now:
                            new_alert = Alert(model_id=plant.id, alert_type=AlertTypes.CLEANSE)
                            Table.ALERT.create(new_alert)
                            alerts_created_count += 1
                            logger.info(f"Created cleanse alert for plant: ID: {plant.id}")
            
            except (ValueError, TypeError) as e:
                logger.warning(f"Could not process care schedule for plant ID: {plant.id}. Error: {e}")
        
        logger.info(f"Finished manage_plant_alerts job. Created {alerts_created_count} new alerts.")
    
    except Exception as e:
        logger.exception(f"An error occurred during the manage_plant_alerts job: {e}")

def _has_alert_type(plants_with_alerts: Dict[str, Alert], plant_id: str, alert_type: AlertTypes) -> bool:
    """Check if plant already has an alert of the specified type."""
    return plant_id in plants_with_alerts and plants_with_alerts[plant_id].alert_type == alert_type
...
def init_scheduler(app: Flask) -> None:
    ...
    for job in scheduler.get_jobs():
        logger.info(f"-> Job: {job.name}, Trigger: {job.trigger}, Next run: {job.next_run_time}")