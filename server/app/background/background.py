...
#             if plant.id in plants_with_alerts or not all([plant.watered_on, plant.watering]):
#                 continue

#             try:
#                 due_date = plant.watered_on + timedelta(days=float(plant.watering))
#                 if due_date < now:
#                     new_alert = Alert(model_id=plant.id, alert_type=AlertTypes.WATER.value)
#                     Table.ALERT.create(new_alert)
#                     alerts_created_count += 1
#                     logger.info(f"Created watering alert for plant: {plant.name} (ID: {plant.id})")
#             except (ValueError, TypeError) as e:
#                 logger.warning(f"Could not process watering schedule for plant {plant.name} (ID: {plant.id}). Error: {e}")
